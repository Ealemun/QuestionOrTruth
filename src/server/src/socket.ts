...
export const initSocket = (httpServer: HttpServer) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      "player:promote_master",
      (roomId: string, targetPlayerId: string, targetPlayerName: string) => {
        if (!isMaster(roomId, socket.id)) {
          console.log(
            `❌ Player ${socket.id} tried to promote a master in room ${roomId}`
          );
          socket.emit("error", { message: "Not authorized" });
          return;
        } else if (targetPlayerId === socket.id) {
          console.log(
            `❌ Player ${socket.id} tried to promote themselves as master in room ${roomId}`
          );
    ...
    socket.on(
      "player:kick_player",
      (roomId: string, targetPlayerId: string, playerName: string) => {
        if (!isMaster(roomId, socket.id)) {
          console.log(
            `❌ Player ${socket.id} tried to kick ${targetPlayerId} in room ${roomId}`
          );
          socket.emit("error", { message: "Not authorized" });
          return;
        } else if (targetPlayerId === socket.id) {
          console.log(
            `❌ Player ${socket.id} tried to kick themselves in room ${roomId}`
          );
